    ...
    <main className="mx-auto max-w-[1600px] px-[3vw] pb-20 pt-14 font-sans max-[700px]:px-3.5 max-[700px]:pt-[38px]">
      ...
      <div className="grid min-h-[680px] grid-cols-[minmax(250px,320px)_minmax(380px,1fr)_minmax(250px,320px)] border border-[var(--line)] bg-[#0c0d14] max-[1000px]:grid-cols-[280px_1fr] max-[700px]:flex max-[700px]:flex-col min-[1001px]:h-[680px] min-[1001px]:min-h-0">
        ...
        <aside className="min-h-0 border-l border-[var(--line)] bg-[#0d0e15] p-[26px] max-[1000px]:col-span-full max-[1000px]:border-l-0 max-[1000px]:border-t max-[1000px]:border-[var(--line)] min-[1001px]:overflow-y-auto">
          ...
          <div className="my-6"><ControlLabel label="Artwork scale" value={`${Math.round(card.artwork.scale * 100)}%`}/><Slider min={50} max={220} value={[card.artwork.scale * 100]} onValueChange={([value]) => patchArtwork({ scale: value / 100 })}/></div>
          <div className="my-6"><ControlLabel label="Horizontal" value={`${Math.round(card.artwork.x)}%`}/><Slider min={0} max={100} value={[card.artwork.x]} onValueChange={([value]) => patchArtwork({ x: value })}/></div>
          <div className="my-6"><ControlLabel label="Vertical" value={`${Math.round(card.artwork.y)}%`}/><Slider min={0} max={100} value={[card.artwork.y]} onValueChange={([value]) => patchArtwork({ y: value })}/></div>
