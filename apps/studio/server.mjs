...
export function isAllowedImageUrl(value) {
  try { const url = new URL(value); return url.protocol === 'https:' && (url.hostname === 'donmai.us' || url.hostname.endsWith('.donmai.us')); }
  catch { return false; }
}
export function normalizeDanbooruQuery(query) {
  const normalized = String(query || '').trim();
  if (!normalized) return 'hololive rating:g';
  return /(?:^|\s)rating:(?:g|s|q|e|general|sensitive|questionable|explicit)(?:\s|$)/i.test(normalized) ? normalized : `${normalized} rating:g`;
}
...
async function fetchDanbooru(url) { return fetch(url, { headers: { 'User-Agent': 'HoloStudio/1.0 (personal art tool)', Accept: 'application/json' } }); }

async function handler(req, res) {
  const requestUrl = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
  if (requestUrl.pathname === '/api/health') return send(res, 200, JSON.stringify({ ok: true }), 'application/json');
  if (requestUrl.pathname === '/api/posts') {
    ...
    try {
      const response = await fetchDanbooru(upstream); if (!response.ok) throw new Error(`Danbooru returned ${response.status}`);
      const posts = await response.json();
      const normalized = posts.map((post) => ({ id: post.id, tag_string_character: post.tag_string_character, tag_string_artist: post.tag_string_artist, image_width: post.image_width, image_height: post.image_height, preview_file_url: post.preview_file_url || post.large_file_url || post.file_url || '', large_file_url: post.large_file_url || post.file_url || post.preview_file_url || '', file_url: post.file_url || post.large_file_url || post.preview_file_url || '' })).filter((post) => post.preview_file_url && post.file_url);
  ...
  if (requestUrl.pathname === '/api/tags') {
    const query = requestUrl.searchParams.get('q')?.slice(0, 80).trim() || ''; if (!query) return send(res, 200, '[]', 'application/json');
    ...
    try {
      const response = await fetchDanbooru(upstream); if (!response.ok) throw new Error(`Danbooru autocomplete returned ${response.status}`);
      const results = await response.json();
      const tags = results.map((item) => ({ name: item.value || item.label || item.name || '', label: item.label || item.value || item.name || '', category: item.category ?? null, post_count: item.post_count ?? null })).filter((item) => item.name);
      return send(res, 200, JSON.stringify(tags), 'application/json');
    } catch (error) { return send(res, 502, JSON.stringify({ error: error.message }), 'application/json'); }
  }
  if (requestUrl.pathname === '/api/image') {
    const source = requestUrl.searchParams.get('url') || ''; if (!isAllowedImageUrl(source)) return send(res, 400, 'Unsupported image URL.');
    try {
      ...
      const contentType = response.headers.get('content-type') || ''; if (!contentType.toLowerCase().startsWith('image/')) throw new Error(`Image host returned non-image content (${contentType || 'unknown'})`);
  ...
  if (vite) return vite.middlewares(req, res, () => send(res, 404, 'Not found.'));

  const pathname = decodeURIComponent(requestUrl.pathname === '/' ? '/index.html' : requestUrl.pathname);
  const base = resolve(root, 'dist'); const baseWithSep = base.endsWith(sep) ? base : `${base}${sep}`; const filename = resolve(base, `.${pathname}`);
  if (filename !== base && !filename.startsWith(baseWithSep)) return send(res, 404, 'Not found.');
  try { if (!(await stat(filename)).isFile()) throw new Error(); return send(res, 200, await readFile(filename), types[extname(filename)] || 'application/octet-stream'); }
