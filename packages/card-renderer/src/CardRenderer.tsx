...
function runWorker(worker: Worker, payload: Record<string, unknown>, transfer: Transferable[] = []) {
  return new Promise<{ buffer: ArrayBuffer; contentType: string }>((resolve, reject) => {
    ...
    worker.addEventListener('error', (event) => {
      cleanup();
      reject(new Error(event.message || 'Card rendering worker failed'));
    }, { once: true });
...
function useSubject(card: CardDefinition, artworkUrl: string, onStatusChange?: CardRendererProps['onStatusChange']) {
  ...
  useEffect(() => {
    ...
    if (!subject?.separated || !artworkUrl) {
      setSubjectUrl('');
      return;
    }
    ...
    void (async () => {
      try {
        ...
        const separated = await runWorker(
          new Worker(new URL('./subject-worker.ts', import.meta.url), { type: 'module' }),
        ...
        const refined = await runWorker(
          new Worker(new URL('./mask-refine-worker.ts', import.meta.url), { type: 'module' }),
          { buffer: separated.buffer, contentType: separated.contentType, settings: subject.mask },
          [separated.buffer],
        );
        if (cancelled) return;
  ...
  }, [artworkUrl, card.artwork.subject?.separated, card.artwork.subject?.mask.threshold, card.artwork.subject?.mask.feather, card.artwork.subject?.mask.expand, onStatusChange]);
...
export function CardRenderer({ card, resolveArtworkUrl = (url) => url, interactive = true, className = '', onStatusChange }: CardRendererProps) {
  ...
  const artworkUrl = useMemo(() => resolveArtworkUrl(card.artwork.url), [card.artwork.url, resolveArtworkUrl]);
  ...
  const style: RendererStyle = {
    '--art-x': `${card.artwork.x}%`, '--art-y': `${card.artwork.y}%`, '--art-scale': card.artwork.scale,
    '--mx': '50%', '--my': '50%', '--posx': '50%', '--posy': '50%', '--hyp': 0, '--rx': '0deg', '--ry': '0deg',
  };
  ...
  function handlePointerMove(event: PointerEvent<HTMLDivElement>) {
    ...
    shell.style.setProperty('--mx', `${x * 100}%`); shell.style.setProperty('--my', `${y * 100}%`);
    shell.style.setProperty('--posx', `${x * 100}%`); shell.style.setProperty('--posy', `${y * 100}%`);
    shell.style.setProperty('--hyp', String(hyp)); shell.style.setProperty('--rx', `${(0.5 - y) * 12}deg`); shell.style.setProperty('--ry', `${(x - 0.5) * 12}deg`);
  }
  ...
  return (
    <div ref={shellRef} className={`holo-card-shell ${flipped ? 'is-flipped' : ''} ${className}`.trim()} style={style}
      onPointerMove={handlePointerMove} onPointerLeave={resetTilt} onClick={toggleFlip}
      onKeyDown={(event) => { if (!interactive || (event.key !== 'Enter' && event.key !== ' ')) return; event.preventDefault(); toggleFlip(); }}
      role={interactive ? 'button' : undefined} tabIndex={interactive ? 0 : undefined} aria-pressed={interactive ? flipped : undefined} aria-label={interactive ? 'Flip card' : undefined}>
      <div className="holo-card-rotator">
        <article className={`card holo-card-face holo-card-front ${card.artwork.mode === 'frame' ? 'in-frame' : ''}`} data-foil={card.appearance.backgroundFoil}>
          ...
          <img className="art-layer art-background" src={artworkUrl} alt={card.artwork.name || 'Card artwork'} />
